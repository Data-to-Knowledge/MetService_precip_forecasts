...
for fprod in Fproducts:
    df_canterbury_stats = pd.DataFrame(columns=['Forecasted hours', 'Accum. hours', 'R-squared [-]', 'RMSE [mm]', 'Nr. of observations'])
    ...
    for fhours in range(1,24+1):
        for ah in accum_hours:
            ...
            df_stats = pd.DataFrame(data={'Forecasted hours': [fhours], 'Accum. hours': [ah], 'R-squared [-]': [R2], 'RMSE [mm]': [rmse_val], 'Nr. of observations': [nrObs]})
            #-concat with final dataframe
    ...
    df_catchment_stats = pd.DataFrame(columns=['Catchment', 'Forecasted hours', 'Accum. hours', 'R-squared [-]', 'RMSE [mm]', 'Nr. of observations'])
    for catch in unique_catchments:
        print('Processing %s' %catch)
        for fhours in range(1,24+1):
            for ah in accum_hours:
                ...
                if flag:
                    #-Put the statistics in the dataframe
                    df_stats = pd.DataFrame(data={'Catchment': [catch], 'Forecasted hours': [fhours], 'Accum. hours': [ah], 'R-squared [-]': [R2], 'RMSE [mm]': [rmse_val], 'Nr. of observations': [nrObs]})
                    #-concat with final dataframe
